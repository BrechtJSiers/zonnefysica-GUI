...
from zonnefysica.controller import Controller
...
class Model:
    ...
    def fitscan(self, order_N, range_1_A, range_2_A, range_1_B, range_2_B):
        data_order_N_A = self.controller.data_A(order_N)
        data_order_N_B = self.controller.data_B(order_N)
        ...
        if order_N == 13:
            ...
            uncertainty_x = [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5]

            # Polynomial fit for wavelength calibration
            fit_order = 2

        # 5 of hoger valt buiten
        fit_1 = np.polynomial.polynomial.polyfit(
            x_list, wavelength_list, fit_order, w=uncertainty_x
        )
        ...
        fit_order_norm = 10
        fit_2_A = np.polynomial.polynomial.polyfit(
            wavelength_object,
            (flux_object_A - dark_A) / (tungstenflat_A - darkflat_A),
            fit_order_norm,
        )
        ...
        fit_2_B = np.polynomial.polynomial.polyfit(
            wavelength_object,
            (flux_object_B - dark_B) / (tungstenflat_B - darkflat_B),
            fit_order_norm,
        )
        ...
        flux_object_norm_A = (flux_object_A - dark_A) / (
            (tungstenflat_A - darkflat_A) * normalisation_fit_A
        )
        ...
        def normal_distribution(x, std, avg, c):
            return (
                -(np.e ** (-(((x - avg) / std) ** 2) / 2)) / (std * np.sqrt(2 * np.pi))
                + c
            )
    ...
    (
        ...
        fit_B_wavelength,
        normal_distribution,
    ) = fitscan(3, range_1_A, range_2_A, range_1_B, range_2_B)
